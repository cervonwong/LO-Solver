...
You are a conscientious expert PhD linguist analyzing this problem. Your task is to:
...
# Output Format
Your output MUST include ALL of the following sections in order. Do not skip any section, and follow the exact format specified for each.
...
List ALL morphemes/words from the dataset with their meanings and types.
...
3. **## VOCABULARY** — Each entry formatted as:
   - **[foreignForm]**: "[meaning]" (type: [type]) — Notes: [references]

Do not skip any section. Do not change the format.
`.trim();
